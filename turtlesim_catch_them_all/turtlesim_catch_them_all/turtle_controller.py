...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        # self.target_x = 4.0
        # self.target_y = 5.0
        # self.catch_closest_turtle_fist = self.get_parameter(
        #     "catch_closest_turtle_fist").value
        self.pose_: Pose = None
        self.turtle_catch = None
        self.cmd_vel_publisher_ = self.create_publisher(
            Twist, "/turtle1/cmd_vel", 10)
        self.pose_subscriber_ = self.create_subscription(
            Pose, "/turtle1/pose", self.callback_pose, 10)
        #creating subscriber to get the position of alive turtles
    ...
    def callback_alive_turtles(self, turtle_list: TurtleArray):
        if len(turtle_list.turtles) > 0:
            ...
            self.turtle_catch = closest_turtle
            #self.turtle_catch = turtle_list.turtles[0]  <to catch in spawn order>
    

    def control_loop(self):
        if self.pose_ == None or self.turtle_catch == None:
            return

        dist_x = self.turtle_catch.x - self.pose_.x
        dist_y = self.turtle_catch.y - self.pose_.y
        distance = math.sqrt(dist_x * dist_x + dist_y * dist_y)
        ...
        if distance > 0.5:
            ...
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
            self.call_catch_turtle_service(self.turtle_catch.name)
    ...
    def callback_call_catch_turtle_service(self, future, turtle_name):
        ...
        if not response.success:
            self.get_logger().error("Turtle" + turtle_name + " couldn't be removed")
